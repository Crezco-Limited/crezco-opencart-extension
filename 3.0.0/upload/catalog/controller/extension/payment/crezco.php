...
class ControllerExtensionPaymentCrezco extends Controller {
	...
	public function index() {
		if ($this->config->get('payment_crezco_user_id')) {
			...
			$setting = array_replace_recursive((array)$config_setting, (array)$this->config->get('payment_crezco_setting'));
			...
			return $this->load->view('extension/payment/crezco', $data);
		}
	}
	...
	public function confirm() {					
		...
		$api_key = $this->config->get('payment_crezco_api_key');
		$partner_id = $this->config->get('payment_crezco_partner_id');
		$user_id = $this->config->get('payment_crezco_user_id');
		$environment = $this->config->get('payment_crezco_environment');
		...
		if (!empty($pay_demand_id)) {
			$payment_info = array(
				...
				'failureCallbackUri' => $this->url->link('extension/payment/crezco/failurePage', '', true)
			);
	...
	public function failurePage() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/crezco/failurePage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('extension/payment/crezco/failure_page', $data));
	}
	...
	public function webhook() {				
		...
		foreach ($webhooks_info as $webhook_info) {
			$this->model_extension_payment_crezco->log($webhook_info, 'Webhook');
