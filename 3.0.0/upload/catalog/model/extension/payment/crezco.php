...
class ModelExtensionPaymentCrezco extends Model {
	...
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_crezco_user_id') && ($this->config->get('payment_crezco_total') > 0 && $this->config->get('payment_crezco_total') > $total) || ($currency_code != 'GBP')) {
			$status = false;
		} elseif (!$this->config->get('payment_crezco_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('payment_crezco_sort_order')
			);
		}
