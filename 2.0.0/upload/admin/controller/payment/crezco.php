...
class ControllerPaymentCrezco extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/crezco', 'token=' . $this->session->data['token'], 'SSL')
		);
						
		$data['action'] = $this->url->link('payment/crezco', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');
		$data['connect_url'] =  str_replace('&amp;', '&', $this->url->link('payment/crezco/connect', 'token=' . $this->session->data['token'], 'SSL'));
		$data['disconnect_url'] =  str_replace('&amp;', '&', $this->url->link('payment/crezco/disconnect', 'token=' . $this->session->data['token'], 'SSL'));
		...
		if (!empty($this->session->data['environment']) && !empty($this->request->request['user-id'])) {						
			...
			$webhook_info = array(
				...
				'callback' => $data['catalog'] . 'index.php?route=payment/crezco/webhook'
			);
		...
		if (VERSION >= '2.2.0.0') {
			$this->response->setOutput($this->load->view('payment/crezco', $data));
		} else {
			$this->response->setOutput($this->load->view('payment/crezco.tpl', $data));
		}
	}
	...
	public function connect() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$partner_url = $this->url->link('payment/crezco', 'token=' . $this->session->data['token'], true);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/crezco')) {
